...
class ShellCalculationInput(BaseModel):
    ...
    tl_tl_length_mm: float = Field(..., gt=0, description="Vessel tangent-to-tangent (T/T) length in mm")
    ...
    bending_allowance_mm: float = Field(default=0.0, ge=0, description="Bending allowance per side in mm")
    density_f2: float = Field(default=7.85, gt=0, description="Material density constant in kg/(m^2*mm)")
...
class ShellCalculationResult(BaseModel):
    ...
    cost: float = Field(
        ..., description="Step 5: Total material cost in currency units"
    )
